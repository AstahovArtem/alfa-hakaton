...
Два прогона с полного сервисного датасета, пары запросов mask+unmask на каждый элемент. Стенд: 1 узел 4 vCPU / 8 GB, k3s, 3-4 реплики (HPA), лимит 1 CPU на под, Redis в кластере. Полные отчёты: [docs/loadtest/report-1000.md](docs/loadtest/report-1000.md) и [docs/loadtest/report-2000.md](docs/loadtest/report-2000.md).

| Прогон | Достигнутый RPS | Пар/с | Доля 429 | Доля неверных round-trip | p50/p95/p99 mask | p50/p95/p99 unmask |
| --- | --- | --- | --- | --- | --- | --- |
| 1000 RPS, 5 мин, https/Traefik | 989.02 | 494.52 | 0.0000% | 0.0020% | 12.98 / 95.46 / 229.97 ms | 12.64 / 88.87 / 184.61 ms |
| 2000 RPS, 3 мин, прямой вход | 1950.40 | 975.28 | 0.0000% | 0.0083% | 21.84 / 74.01 / 133.85 ms | 17.68 / 58.16 / 90.97 ms |

Достигнутый RPS чуть ниже целевого на обоих прогонах: генератор открытой модели не подгоняет результат, а честно считает то, что реально ушло на сеть. Ошибки по коду 0 (10 и 59 штук на прогон) это оборванные соединения, не ответы сервиса. Traefik на TLS тратит около ядра, поэтому чекеру даём прямой вход через LoadBalancer без прокси.
