...
func TestDateWordsDetect(t *testing.T) {
	cases := []struct {
		...
		{"compoundTensSeventySix", "двадцать третьего февраля тысяча девятьсот семьдесят шестого", "двадцать третьего февраля тысяча девятьсот семьдесят шестого"},
		{"compoundTensSixtyTwo", "десятого октября тысяча девятьсот шестьдесят второго года", "десятого октября тысяча девятьсот шестьдесят второго года"},
		{"compoundTensTwentyThree", "двадцать пятого декабря две тысячи двадцать третьего года", "двадцать пятого декабря две тысячи двадцать третьего года"},
		{"compoundTensTwentyFive", "третьего сентября две тысячи двадцать пятого года", "третьего сентября две тысячи двадцать пятого года"},
		{"compoundTensTwentySix", "двадцать второе октября две тысячи двадцать шестого года", "двадцать второе октября две тысячи двадцать шестого года"},
		{"uppercaseStartOfReply", "Клиент: Двадцать третьего февраля тысяча девятьсот семьдесят шестого.", "Двадцать третьего февраля тысяча девятьсот семьдесят шестого"},
	}
	for _, c := range cases {
